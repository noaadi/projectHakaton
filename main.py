...
class MainMenu:
    ...
    def render(self):
        ...
        for label, rect in self.buttons:
            ...
            label_text = f"PLAY {label.upper()}"
            max_width = self.button_width - 10
            max_height = self.button_height - 10
            font_size = 16

            while font_size >= 8:
                scaled_font = pygame.font.Font(os.path.join("assets", "press_start.ttf"), font_size)
                text_surface = scaled_font.render(label_text, True, self.text_color)
            ...
            if font_size < 8:
                scaled_font = pygame.font.Font(os.path.join("assets", "press_start.ttf"), 8)
                text_surface = scaled_font.render(label_text, True, self.text_color)
                text_rect = text_surface.get_rect()

            text_rect.center = scrolled_rect.center
